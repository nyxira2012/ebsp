...
from src.pve.models import PveSessionData, PveSquadState, MapGraph, PveEntityState
from src.pve.enums import SessionStatus, ExitMethod
...
async def test_reward_controller_finalize_success():
    ...
    graph = MapGraph(width=1, height=1)
...
async def test_reward_controller_defeated():
    ...
    squad_state = PveSquadState(members=members, locked_config={})
    graph = MapGraph(width=1, height=1)
    session_data = PveSessionData(session_id=777, user_id=1, region_id="r1", map_graph=graph, squad_state=squad_state, created_at=0, last_heartbeat=0)
