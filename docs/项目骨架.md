...
| ⚠️ src/models.py | 805 | 319 | — |
| ⚠️ src/skills.py | 450 | 49 | — |
| ⚠️ src/combat/engagement.py | 317 | 21 | — |
| ⚠️ src/combat/statistics_collector.py | 282 | 68 | — |
| ⚠️ src/user/schemas.py | 233 | 124 | — |
| ⚠️ src/api/presentation_api.py | 205 | 21 | — |
| ⚠️ src/presentation/contracts.py | 179 | 88 | — |
| ⚠️ src/presentation/template.py | 111 | 54 | — |
| ⚠️ src/pve/schemas.py | 88 | 55 | — |

## 仓库根（Python）

**结构模块**（被别的模块 import，改之波及大——列签名）

### src/models.py（被依赖 31）
...
class MechaConfig(BaseModel)
    ...
    init_armor: int
    init_mobility: int
_… +304 更多（`outline.py src/models.py` 看全）_

### src/database/models.py（被依赖 13）
class User(Base, TimestampMixin, SoftDeleteMixin)
    ...
    pve_progress: Mapped[Optional['UserPveProgress']]
_… +69 更多（`outline.py src/database/models.py` 看全）_
...
_… +4 更多（`outline.py src/loader.py` 看全）_
...
class UserAssetRepository
    ...
    async def update_mecha_upgrades(session: AsyncSession, user_mecha_id: int, upgrades: dict) -> Optional[UserMecha]
_… +5 更多（`outline.py src/user/repository.py` 看全）_

### src/user/schemas.py（被依赖 7）
...
class MechaUpgrades(BaseModel)
    hp: int
_… +109 更多（`outline.py src/user/schemas.py` 看全）_
...
async def get_optional_user(credentials: Optional[HTTPAuthorizationCredentials]=Depends(optional_security), session: AsyncSession=Depends(get_async_session)) -> Optional[User]

### src/user/inventory.py（被依赖 4）
...
class BattleEntryService
    async def build_debug(loader: Any, req: 'BattleRequest', user: 'User | None', db_session: 'AsyncSession') -> EngagementSpec
...
class ParticipantProfile(BaseModel)
    ...
    en: int
    max_en: int
_… +73 更多（`outline.py src/presentation/contracts.py` 看全）_
...
_… +40 更多（`outline.py src/pve/schemas.py` 看全）_
...
class RewardController
    ...
    async def finalize(cls, db: AsyncSession, session_data: PveSessionData, exit_method: ExitMethod, inventory_service: 'InventoryService', mothership_config: 'MothershipConfig', loader: Any) -> Dict[str, Any]
...
- src/api/inventory_api.py（4 签名）
- src/api/pve_api.py（9 签名）
- src/api/user_api.py（10 签名）
