...
```
tests/
├── conftest.py                    # 【核心】所有通用 Fixtures（机体、驾驶员、武器、战斗上下文）
├── test_combat_resolver.py        # 圆桌判定与伤害计算
├── test_complex_scenarios.py      # 复杂战斗场景
├── test_engine_initiative.py      # 先手判定系统
├── test_event_manager_isolation.py # EventManager 隔离测试
├── test_factory.py                # 快照工厂
├── test_integration_complex.py    # 大型集成测试
├── test_loader.py                 # 数据加载器
├── test_presentation.py           # 战斗演出系统
├── test_presentation_showcase.py  # 演出系统展示
├── test_resolver_coverage.py      # 圆桌判定覆盖
├── test_side_effects.py           # 副作用系统
├── test_skill_callbacks.py        # 技能回调函数
├── test_skill_event_system.py     # 技能事件系统
├── test_skills_conditions.py      # 技能条件系统
├── test_skills_integration.py     # 技能集成测试
├── test_skills_processor.py       # EffectProcessor
├── test_statistics_collector.py   # 战斗统计收集器
├── test_trait_system_pytest.py    # 特性系统
├── test_unit_models.py            # 数据模型单元测试
├── test_weapon_tags_debug.py      # 武器标签调试
└── pytest.ini                     # 配置文件（asyncio_mode = auto）
```
