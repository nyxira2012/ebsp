...
### 7.1 Phase 1: 补充钩子点（基础） ✅ 已完成

**文件**: `src/combat/resolver.py`, `src/combat/engine.py`

**已实现的钩子点**：
1. `HOOK_PRE_MISS_RATE`（未命中率）
2. `HOOK_PRE_PARRY_RATE`（招架率）
3. `HOOK_PRE_BLOCK_RATE`（格挡率）
4. `HOOK_POST_ROLL_RESULT`（判定后修正）
5. `HOOK_ON_DAMAGE_TAKEN`（伤害抵消）
6. `HOOK_ON_KILL`（击坠）
7. `HOOK_ON_ATTACK_END`（攻击结束）

### 7.2 Phase 2: 扩展 Effect 模型（核心） ✅ 已完成

**文件**: `src/models.py`

**已实现**：
1. `Effect` dataclass 包含完整字段定义（id, name, source_id, hook, operation, value, priority, sub_priority, trigger_chance, target, duration, charges, consume_charges, conditions, side_effects, payload）
2. `Condition` 和 `SideEffect` dataclass 定义
3. `TriggerEvent` 和 `BuffState` 数据模型

### 7.3 Phase 3: 创建 EffectProcessor（关键） ✅ 已完成

**文件**: `src/skill_system/processor.py`

**已实现**：
1. `EffectProcessor` 类
2. `process_effects()` 方法（包含排序、概率、操作运算）
3. 与 `SkillRegistry` 集成

### 7.4 Phase 4: 实现回调函数（演示） ✅ 已完成

**文件**: `src/skills.py`（回调函数在 `SkillRegistry` 装饰器下注册）

**已实现的回调函数**：17 个（详见第 9.2 节回调函数列表）

### 7.5 Phase 5: 数据驱动配置 ✅ 已完成

**文件**: `data/skills.json`, `data/skills_extended.json`, `src/skill_system/effect_factory.py`, `src/loader.py`

**已实现**：
1. 技能配置 JSON（`skills.json` 25KB + `skills_extended.json` 31KB）
2. `EffectFactory` 类（从 JSON 配置创建 Effect 对象）
3. 在 `DataLoader` 中集成
