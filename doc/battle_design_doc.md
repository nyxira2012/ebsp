...
气力提供小幅的稳定性补正，确保高战意方获得优势。
`命中/躲闪附加率 = (气力 - 100) * 0.2%` (即 150 气力时提供额外 10% 补正)。
...
*   **计算公式**: `实际防御概率 = 基础概率 * (1 - 攻击方精准削减比)`
*   *精准作用*: 攻击方的“精准”属性越高，防御方在圆桌中的躲闪、招架、格挡区间就越窄，剩余的“命中/暴击”区间越大。
...
### 6.2 驾驶员属性 (Pilot Stats)
| 属性名                 | 代码             | 说明                                                                        |
| :--------------------- | :--------------- | :-------------------------------------------------------------------------- |
| **射击值 (Shooting)**  | `stat_shooting`  | 影响射击类武器（步枪、狙击炮）的命中率与最终伤害修正。                      |
| **格斗值 (Melee)**     | `stat_melee`     | 影响格斗类武器（光束剑、热能斧）的命中率与最终伤害修正。                    |
| **觉醒值 (Awakening)** | `stat_awakening` | 影响特殊武器（如浮游炮）的可用性及命中补正，同时也影响直觉回避。            |
| **守备值 (Defense)**   | `stat_defense`   | 影响防御时的减伤效果（增加“有效生命值”）及被暴击率抗性。                    |
| **反应值 (Reaction)**  | `stat_reaction`  | 影响机体的躲闪、招架、格挡以及决定战斗的**先攻权判定**（基底A的重要组成）。 |

### 6.3 武器属性 (Weapon Stats)
| 属性名           | 代码    | 说明                                                                   |
| :--------------- | :------ | :--------------------------------------------------------------------- |
| **威力 (Power)** | `power` | 武器的基础伤害数值。                                                   |
| **消耗 (Cost)**  | `cost`  | 每次攻击消耗的资源，通常为弹药 (Ammo) 或能量 (EN)。                    |
| **射程 (Range)** | `range` | 武器的有效攻击距离区间（如 0-2000m），决定了在特定交战距离下的可用性。 |
