...
## 2026-09-22 编队暂不开放：出战机体=玩家持有首机（Doc 7 v2.4 + Doc 16 v1.6 + Doc 14 v1.10）

> **项目快照**：用户裁决「机体编队功能目前暂时不要开放，所有对战和练习都必须使用玩家配备的机体（有机体就开自己的，没有就要求玩家前往整备库驾驶可出动机体）」| simulate 存档覆盖通道：编队门槛退役、持有门槛上位、两段式演示局废除 | 起因：用户实测练习场发现「有机体但无编队」账号被 NO_ACTIVE_SQUAD 拦在存档出击外（ebs-duo 侧先以 DB 补编队临时解，本批改规则根治）

### 改动
- `entry.build_debug`：存档覆盖自「出战编队 mecha_ids[0/1]」改为「持有清单取首台/次台」；`owned_mechas` 预取透传替代 `active_squad`。`list_user_mechas` 增补 id 升序——「持有首台」的次序依据（无序 select 在 SQLite 恰好按 rowid 返回，不钉死是隐式依赖）。
- `presentation_api.simulate`：`assert_squad_ready` 拆除，改「无机体 → 400 STARTER_NOT_CLAIMED」单码分流（函数保留给编队 API 演进位，战斗路径零调用）。
- **Doc 14 v1.10（本批隐藏收获）**：测试失败暴露 `init.mecha_id` 对存档机体是 `user_*` 实例 id——前端立绘对照表（§9.1）以配置 ID 为键，实例 id 查不到 → **自己的机体在战斗页会没图**（ebs-duo r2 只验了文字没验图，漏网）。快照增补 `config_id`、init 块增补 `mecha_config_id`（非破坏性，金样张重录）；存档降级识别因此从启发式变精确（实例 id 与配置 id 命名空间天然不重叠）。
- 测试：`test_battle_entry` 降级用例桩改持有清单 + 新增「持有首台覆盖」单测；`test_user_onboarding` 新增「零编队照样开自己的机体」「无机体 400」两 API 回归；全量 pytest 绿。

### 教训
- **契约键的语义漂移要靠「第一个真实消费者」暴露**：`instance_id` 自 v2.2 起就分叉（配置/存档两形态），金样张走纯配置路径看不见；直到存档机体真进了 init 块才炸出立绘断链。跨形态字段的增补（config_id）应在分叉引入时同步做，而不是等消费者踩坑。
